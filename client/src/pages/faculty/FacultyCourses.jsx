...
import { getStudentsForCourse } from "../../services/userRolesApi.js";
...
const FacultyCourses = () => {
    ...
    const handleViewStudents = async (courseId) => {
        ...
        try {
            const res = await getStudentsForCourse(courseId, selectedYear);
            setEnrolledStudents(res.data);
            setActiveCourseId(courseId);
        } catch (err) { toast.error("Greška pri učitavanju studenata."); }
    };

    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold">Moji Predmeti i Studenti</h1>
                <select
                    value={selectedYear}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {courses.map(c => (
                        <Card key={c.id}>
                            <div className="flex justify-between items-center">
                                ...
                                <div className="w-32">
                                    <Button onClick={() => handleViewStudents(c.id)} variant="secondary">
                                        Vidi studente
                                    </Button>
                                </div>
                            </div>
                ...
                {activeCourseId && (
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-green-100 h-fit">
                        <h2 className="text-xl font-bold mb-4 text-green-600">Upisani Studenti</h2>
                        {enrolledStudents.length === 0 ? (
                            <p className="text-gray-500 text-sm">Nema upisanih studenata u ovoj godini.</p>
                        ) : (
                            <ul className="flex flex-col gap-3 max-h-96 overflow-y-auto">
                                {enrolledStudents.map(s => (
                                    <li key={s.id} className="p-3 bg-gray-50 rounded-lg flex justify-between items-center border border-gray-100">
                                        <div>
